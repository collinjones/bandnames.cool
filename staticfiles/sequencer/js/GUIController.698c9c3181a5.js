class GUIController {
    ...
    changeObjectType() {
        ...
        if (this.currentObjectDrawType == "Container") {
            ...
            this.gui.showControl("Container ID")
            ...
            this.gui.hideControl("Container ID")
    ...
    changeSideLength() {
        var selectionID = Number(this.getValue("Container ID").value) - 1
        for (const container of this.simulation.containers) {
            if (container.container.label == selectionID) {
    ...
    changeContainers(){
        ...
        for (const container of this.simulation.containers) {
            console.log(
                "Container Label: ", container.container.label
            )
            if (container.container.label == selectionID) {
                container.selected();
            } else {
    ...
    changeMIDIInput() {
        this.simulation.MIDIIn_controller.changeMIDIIn(
            this.settings.getValue("MIDI Input Device").value
        )
    }
    changeMIDIOutput() {}
    backgroundColor() {}
    ...
    updateContainersList() {
        this.gui.removeControl("Container ID")
        let containerIDs = ["New Container"]

        for(const container of this.simulation.containers) {
            containerIDs.push(Number(container.container.label) + 1)
        }
