class GUIController {
    ...
    changeSideLength() {
        for (const container of this.simulation.containers) {
            container.updateSideLength(
                map(this.getValue("Side Length"), 1, 10, 25, 250)
            )
        }
    }
    ...
    changeContainerSides() {
        for (const container of this.simulation.containers) {
            let perimeter = this.getValue("Sides") * container.sideLength
            let newLength = perimeter / this.getValue("Sides");
            container.updateSides(this.getValue("Sides"))
            container.updateSideLength(newLength)
        }
    }
    changeContainerSize() {
        for (const container of this.simulation.containers) {
            let containerPartLength = container.sideLength
            container.updateSize(map(this.getValue("Container Size"), 1, 10, 25, 300))
        }
    }
